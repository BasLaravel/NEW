...
<body>
  ...
  <nav class="navbar navbar-expand-lg navbar-dark">
    <div class="navbar-header ml-0 mt-4">
      <a class="navbar-brand" href="index.php"><img class = "Logo" src="{{asset('img/LogoNEW.png')}}" alt="" style = "max-width:175px;max-height:175px;">
      </a>
       ...
       @auth
        ...
        <li class="nav-item dropdown" id= "dropdown1">
          <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            <div class="itemcount">@inject('test', 'Gloudemans\Shoppingcart\Cart'){{$test->count()}}</div><img src="{{asset('img/Winkelwagentje.png')}}" style="max-width:49px;max-height:49px;"></a>
          <div id= "dropdown-menu4" class="dropdown-menu dropdown-menu-right" aria-labelledby="navbarDropdown">
              @if ($test->count() == 0)
              @else
              @foreach($test->content() as $laptop)
              <div class="row" id="cart" style="width: 23rem;">
                <div id="headcart" class="col-xs-1 ml-auto w-75">
                  <p>{{str_limit($laptop->options->product->title, $limit = 30, $end ='...')}}</p>
                </div>
                <div class="col-xs-4">
                  <a id= "knop" href="{{ route('cart.destroy', [$laptop->rowId]) }}">x</a>
                </div>
                <div class="col-xs-1">
                  <img class="" src="{{$laptop->options->product->image_large}}"  alt="Product image cap" style="max-width: 100px;">
                </div>
                <div class="col-xs-3 w-50">
                    <p class="beschrijving">{{str_limit($laptop->options->product->short_description, $limit = 70, $end ='...')}}</p>
                </div>
                <div class="col-xs-4">
                  <p id= "prijs"> prijs €{{$laptop->options->product->price}},-</p>
                </div>
              </div>
              @endforeach
              @endif
